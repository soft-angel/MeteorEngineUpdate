...
if($arFile = $arFileAll = glob(PATCH_USER . '/*.php')){
	...
	foreach ($arFile as $file){
		...
		foreach($arAllUser as $arUser){
			if($arUser["OS"])
				$arOsChart[] = $arOs[$arUser["OS"]]["TYPE"];
		}
	}
	if($arOsChart)
		$arOsCount = array_count($arOsChart);
	 // print_r($arBrowserCount);
?>
<div style="margin-bottom: 10px;color: #1FBBA6;text-align: center;"><form method="get">
Диаграмма популярности устройств за <select name="limit" onchange="this.form.submit()">
...
var data = {
    labels: [<?foreach($arOsCount as $name => $value){?>"<?=$name?>", <?}?>],
    datasets: [
        {
